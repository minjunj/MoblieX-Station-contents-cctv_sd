...
async def run_subprocess_async(image_path, function_name):
    """Asynchronously execute a subprocess with the given image path and function name."""
    proc = await asyncio.create_subprocess_exec(
        'python', 'pub_openark.py', image_path, '--function_name', function_name,
        stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)

    stdout, stderr = await proc.communicate()
    
    if stdout:
        print(f"[{function_name}] Output: {stdout.decode()}")
    if stderr:
        print(f"[{function_name}] Error: {stderr.decode()}")
...
async def capture_and_process_frame(frame, detector, predictor):
    ...
    if len(result) != 0:
        print("Shot CCTV.SD")
        await run_subprocess_async('output_image.jpg', 'cctv.sd')
    else:
        print("Shot CCTV.Default")
        await run_subprocess_async('output_image.jpg', 'cctv.default')
